...
Alternatively, the `TwitterDMTools` package can be installed by downloading the package contents from the Github repository [`https://github.com/jakubk28/TwitterDMTools`](https://github.com/jakubk28/TwitterDMTools) and using the `install.packages()` function to install the package from a local folder.
...
For reproducability, the data collection has been contucted before hand, using the search query `"palm oil,plastic pollution"`, the final data file called `ConservationTweets` is included with the package as a guide for the data mining process.
...
```{r message=FALSE,warning=FALSE,fig.align="center",fig.width=6,fig.height=6,cache=TRUE}
...
Sentiment anaylsis is the process of extracting emotional context from a corpus, in this case, the emotions felt by writers within their tweets. The concept behind sentiment analysis is that most words have either a positive or negative conotation; by comparing the frequencies of these words, either locally within a tweet or globally within the corpus, it is possible to analyse the overall sentiment felt by the author or authors respecitvely. 
...
Where $P(w_i\mid d)$ is the probability of the ith word $w_i$ of the given document $d$, this is then determined by summing the probability of a word $w_i$ appearing within topic $j$ ($\beta$) and the probability of picking a word $w_i$ from the document $d$ ($\gamma$) over the total number of specified topics $K$.
